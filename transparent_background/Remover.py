    ...
    def __init__(self, fast=False, jit=False, device=None, ckpt=None):
        ...
        if jit:
            ...
            try:
                ...
                traced_model = torch.jit.trace(self.model, torch.rand(1, 3, *self.meta['base_size']).to(self.device),
                                               strict=True)
                del self.model
                self.model = traced_model
...
def console():
    ...
    frame_progress = tqdm.tqdm(total=len(loader), position=1 if (_format == 'Video' and len(loader) > 1) else 0,
                               leave=False, bar_format='{desc:<15}{percentage:3.0f}%|{bar:50}{r_bar}')
    sample_progress = tqdm.tqdm(total=len(loader), desc='Total:', position=0,
                                bar_format='{desc:<15}{percentage:3.0f}%|{bar:50}{r_bar}') if (
                _format == 'Video' and len(loader) > 1) else None
    writer = None
    ...
    for img, name in loader:
        ...
        if _format == 'Video' and writer is None:
            writer = cv2.VideoWriter(os.path.join(save_dir, '{}.mp4'.format(outname)), cv2.VideoWriter_fourcc(*'mp4v'),
                                     loader.fps, img.size)
